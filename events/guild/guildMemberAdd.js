...
module.exports = async (bot, member) => {
    ...
    if(member.guild.id == idtable.guilds.wipink){
        let welcomeWipinkEmb = new Discord.RichEmbed()
            ...
            .setDescription('Eu sou BOTete, e estarei lhe servindo nesse discord. Para ver minha lista de comandos, basta digitar `.help`, e se precisar de ajuda com algum comando, digite `.help` e em seguida o comando para qual deseja receber ajuda.')
            ...
            .addField('Entediado?', 'No Cantinho do BOT, eu providencio conteúdo programado para entretenimento. Recomendo que você mute estes canais (desligue as notificações), pois eu spammo bastante coisa lá, e as notificações podem aborrecer você.\nTente sempre que possível usar o canal '+`${bot.channels.get(idtable.channels.wipinkmusic)}`+' quando for pedir pra eu tocar música, assim os outros canais não ficam tão poluídos. Quando for pedir música, certifíque-se antes de que eu não esteja já tocando música em outro canal.\nSe for usar algum comando meu listado como NSFW, utilize as salas correspondentes para esse conteúdo.')
            .setTimestamp()
